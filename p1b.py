...
In order to train the algorithm and save it, call the program using "p1b.py --save WEIGHTS_FILE".
To see the results of previous training, call the program using "p1b.py --load my_weights2".
The program uses the same Convolutional Neural Network (CNN) for the two images, and uses contrastive loss and the Adam
optimizer.
'''
...
epochs = 20 # select the amount of epochs
...
class cnn(nn.Module):
    ...
    def forward(self, image1, image2):
        ...
        results = F.pairwise_distance(image1, image2) # to calculate contrastive loss
...
def accuracy(label, output): # calculate accuracy by setting a threshold and comparing the output to the threshold value
    counter = 0
    result = 0
    classification = np.zeros((label.size()[0],1))
   
    for i in range(0, label.size()[0]):
        if (output.data.cpu().numpy()[i] <= 1.414): # the threshold is the square root of 2
...
def training(cnn_model):
    ...
    for each in dataloader: # for each pair of images loaded
        ...
        optimizer.zero_grad()
        loss = torch.mean((label) * torch.pow(output, 2) + (1 - label) * torch.pow(torch.clamp(margin - output, min=0.0), 2))
        loss.backward()
        optimizer.step()
